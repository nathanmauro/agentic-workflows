...
// In PLAN mode the APPROVAL phase overrides recon's acceptanceCriteria with Nathan's approved version before launch.
export const meta = {
...
(1) Never commit as anyone but Nathan — do NOT add any Co-Authored-By trailer naming Claude/Codex/a model, do NOT add "Generated with" lines.
(2) Before editing, run \`git status\` and remember which files were ALREADY dirty — those are Nathan's in-flight work and are off-limits.
(3) Create and checkout a NEW **fleet iteration specific branch** named exactly \`fleet/round-${ROUND}-<kebab-case-short-task-title>\` (e.g. fleet/round-2-add-user-auth). Stack off the previous fleet round's branch for this project if it exists.
...
(6) After the green verify, run \`bash ~/.claude/skills/codex-fleet/fleet-spec.sh render ${projectName}\` and stage \`docs/fleet/spec.md\` with the slice. Stage ONLY files you created or modified: run \`git diff --name-only\` / \`git status\` first and \`git add\` those exact paths. NEVER use \`git add -A\`, \`git add .\`, or \`git add --update\`. Do NOT stage .claude/.firecrawl/.idea or any file that was already dirty before you started. Commit to the fleet branch only. Do NOT merge to main.
(7) **Artifacts for review (leave behind when useful)**:
   ...
   Only record things that are genuinely useful for Nathan to review the changes on this fleet branch. Do not leave long-running things that will collide if not cleaned.
(8) At the very end, after the commit and artifact recording, run \`bash ~/.claude/skills/codex-fleet/fleet-open-pr.sh ${projectName} ${ROUND} <the exact branch you created>\`. Do NOT push or open PRs by hand; the helper owns push-vs-local-only and draft PR decisions.
...
- Is it well-scoped for ONE autonomous session (not too big, not blocked on a decision Nathan must make)?
