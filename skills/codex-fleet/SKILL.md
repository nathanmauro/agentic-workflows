...
name: codex-fleet
description: >
  Use when Nathan wants to advance several local code projects at once over
  ...
  next vertical slice and Nathan approves/redirects/skips through Claude Code's native
  option prompts on the terminal or his phone — with mid-flight clarifying questions
...
- **PLAN** (default) — Claude proposes each project's next slice from its living spec;
  Nathan **approves / redirects / skips** before any code runs via Claude Code's **native
...
non-danger-flagged repos a **draft PR** is opened per slice (stacked); Nathan reviews
...
```
cd ~/Developer/proj/cockpit
./agents/claude/skills/codex-fleet/start-fleet.sh   # or the ~/.claude/... symlink
```
...
the slice with `"you asked Q, Nathan answered A, continue"` once Nathan replies. The
...
pushes and opens a **draft** PR stacked on the previous slice's branch (Nathan sole
...
- **Identity:** commit as Nathan only. NO `Co-Authored-By` naming Claude/Codex/a model,
...
- **Push/PR (relaxed by decision):** push + **draft** PR for tier-allowed,
  non-danger-flagged repos with a remote; **Nathan owns every merge**. Danger-flagged /
...
- **Autonomy:** the launcher uses `-a never`; full local autonomy on a branch. This
  skill is a **manual trust boundary** — Nathan invokes it explicitly, which IS the
  authorization.
- **Per-repo dangers** travel in the project `hint` and the spec (e.g. mcp-memory-agent:
  origin is PUBLIC; the `experiment/dna-residual-memory` branch + sibling worktree carry
  personal data — NEVER touched or pushed).

## Watch for the deviation pattern

A Codex session may fold pre-existing dirty files into its commit (observed: sba-agentic
folded `SessionsPage.tsx` + `theme.css`). The commit gate diffs `git status --porcelain`
...
| `fleet-prompt.sh` | Builds the native `AskUserQuestion` payloads — `approval <round>` and `clarify` — so Nathan steers with option prompts + freeform, not typed grammar (fixture-tested for the tool's shape limits) |
...
- Falling back to the typed `1 approve` / `1 skip` grammar when `AskUserQuestion` can reach
  Nathan → that grammar is for headless hosts only. The native option prompt (built by
