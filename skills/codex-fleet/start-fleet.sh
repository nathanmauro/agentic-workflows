#!/usr/bin/env bash
# start-fleet.sh
# Interactive launcher for codex-fleet, fired from the cockpit directory.
...
#   ./agents/claude/skills/codex-fleet/start-fleet.sh
#
# The symlinked version in ~/.claude/skills will auto-resolve to source.

set -euo pipefail

# Fire off from cockpit directory
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Resolve to actual source if symlinked
if [ -L "$SCRIPT_DIR" ]; then
  SCRIPT_DIR="$(cd "$(dirname "$(readlink "$SCRIPT_DIR")")" && pwd)"
fi
COCKPIT_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"   # cockpit/agents/claude/skills/codex-fleet -> cockpit

echo "Firing codex-fleet from cockpit dir: $COCKPIT_DIR"
cd "$COCKPIT_DIR"
...
if [ "$MODE" = PLAN ]; then
  MODE_EXPLANATION="PLAN mode runs RECON -> APPROVAL -> RUNNING -> REVIEW for each round; Nathan approves, redirects, or skips each proposed slice through a native option prompt (AskUserQuestion) on terminal or phone before launch."
else
...
# codex-fleet (generated by start-fleet.sh from cockpit dir)
...
Follow ~/.claude/skills/codex-fleet/loop-prompt.md as the authority for the MODE/PHASE machine, including APPROVAL, clarification polling, board refresh, REVIEW, and DONE handling.
...
- Use the exact guardrails from the recon-workflow (Nathan author, selective stage, green verify only, no merge; fleet-open-pr.sh owns push/PR decisions).

Now start the cycle: read state, enter PHASE==RECON logic for ROUND 1.
EOF

echo
echo "=== Fleet ready (fired from cockpit) ==="
...
echo
echo "Run this script again from cockpit for a new fleet with different projects/rounds."
