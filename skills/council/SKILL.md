...
description: >
  ...
  a neutral synthesis and a numbered pick. Use when Nathan says "$council", "convene the
...
This skill documents the council workflow and expected artifacts. The private Cockpit runner is not included in v0. To use this publicly, wire the same protocol to your own runner: one context pack, independent model opinions, anonymized cross-review, neutral synthesis, and a final human pick.

## Overview

Same engine as Claude's `/council`. `scripts/cockpit-council` sends an identical context
pack to three frontier models as fresh one-shot CLI calls (round 1: opinions), then shows
...
nohup /Users/nathan/Developer/proj/cockpit/scripts/cockpit-council run \
...
7. **Capture.** When the user picks (or accepts a consensus that settles a real
   decision), capture via the `sba-agentic` `captureDecision` tool (`source: "codex"`,
   `repo`: current repo): decision, rationale, rejected alternatives with their backing
   voices, and the run dir. If the tool is unavailable, note that in one line — do not
   block.
...
- Engine missing → report the path
  `/Users/nathan/Developer/proj/cockpit/scripts/cockpit-council` and stop.
